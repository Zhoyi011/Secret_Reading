...
import { ArrowLeft, User, Calendar, BookOpen, Clock, Shield, Heart, Eye, MessageSquare, Loader2, Bookmark, CheckCircle2, X } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-8 text-left animate-fade-in relative">
      ...
      <div className="bg-white rounded-3xl border border-gray-100 p-6 sm:p-10 shadow-sm flex flex-col md:flex-row items-center gap-6 sm:gap-8 relative overflow-hidden">
        <div className="absolute top-0 right-0 h-40 w-40 bg-gradient-to-br from-indigo-50 to-transparent rounded-full -mr-16 -mt-16 pointer-events-none"></div>
        ...
        <div className="space-y-3 text-center md:text-left flex-grow">
          <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-3">
            ...
            <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${
              author.role === 'owner' ? 'bg-indigo-100 text-indigo-800' :
              author.role === 'author' ? 'bg-emerald-100 text-emerald-800' :
              'bg-gray-100 text-gray-700'
            }`}>
              <Shield className="h-3 w-3" />
              {author.role === 'owner' ? '站长 (Owner)' :
               author.role === 'author' ? '专栏作者 (Author)' : '专享读者 (Reader)'}
            </span>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 text-xs font-semibold pt-1">
            <button
              ...
              className="flex items-center gap-1 text-gray-600 bg-gray-50 hover:bg-gray-100 border border-gray-100 px-3 py-1 rounded-lg transition-colors cursor-pointer"
            >
        ...
        <div className="flex flex-col gap-4 w-full md:w-auto shrink-0 text-center sm:text-left items-stretch md:items-end">
          <div className="grid grid-cols-2 divide-x divide-gray-100 bg-gray-50/50 rounded-2xl border border-gray-100 p-3 text-center">
            <div className="px-3">
              <span className="block font-display text-base font-bold text-gray-900">{posts.length}</span>
              <span className="text-[10px] text-gray-500">公开发表</span>
            </div>
            <div className="px-3">
              <span className="block font-display text-base font-bold text-gray-900">{totalReceivedLikes}</span>
              <span className="text-[10px] text-gray-500">所获赞数</span>
            </div>
          </div>
